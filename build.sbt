...
def publishVersion = if (isRelease) releaseVersion
else if (isGithubActions) "2.9.0." + githubRunNumber + "-SNAPSHOT"
else "0.0.0-LOCAL"
...
val FunctionalStreamsVersion = "3.14.0"
...
lazy val commonScalaVersionSettings = Seq(
  ...
  crossScalaVersions := Seq("2.12.21", "2.13.18", ScalaVersion)
)
...
lazy val commonSettings = Seq(
  ...
  Compile / doc / scalacOptions := (Compile / doc / scalacOptions).value
...
lazy val pomSettings = Seq(
  ...
  scmInfo := Some(
    ScmInfo(
      url("https://github.com/CleverCloud/pulsar4s"),
      "scm:git:git@github.com:CleverCloud/pulsar4s.git"
    )
  ),
...
lazy val cats_effect =
  Project("pulsar4s-cats-effect", file("pulsar4s-cats-effect"))
    .dependsOn(core)
...
lazy val avro = Project("pulsar4s-avro", file("pulsar4s-avro"))
  ...
  .settings(
    libraryDependencies ++= Seq(
      "com.sksamuel.avro4s" %% "avro4s-core" % (CrossVersion
...
lazy val akka_streams =
  Project("pulsar4s-akka-streams", file("pulsar4s-akka-streams"))
    .dependsOn(core)
...
lazy val pekko_streams =
  Project("pulsar4s-pekko-streams", file("pulsar4s-pekko-streams"))
    .dependsOn(core)
    ...
    .settings(
      ...
      libraryDependencySchemes ++= (CrossVersion
